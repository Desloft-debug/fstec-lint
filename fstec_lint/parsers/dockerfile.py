...
def _heredoc_body(lines: list[str], index: int, buffer: str) -> tuple[list[str], int]:
    ...
    Если ограничитель не встретился до конца файла, значит '<<' было не
    началом heredoc, а частью самой команды ('echo "a << b"'). Тогда не
    поглощаем ни строки и возвращаем исходную позицию: иначе весь
    остаток Dockerfile молча уезжает в аргументы одной инструкции — со
    всеми находками, которые в нём были.
    """
    body: list[str] = []
    cursor = index
