...
Линтер конфигураций для тех, кто готовит инфраструктуру к аттестации.
Разбирает `docker-compose.yml`, `Dockerfile`, `postgresql.conf`,
`pg_hba.conf`, `sshd_config` и юниты systemd — **41 правило**, каждое со
ссылкой на пункт нормативного документа.

Docker Bench, Lynis, Trivy и Checkov находят те же проблемы и мапят их на
CIS Benchmarks или NIST. Вопрос «какую меру приказа ФСТЭК закрывает этот
фикс» остаётся без ответа, а он и нужен перед аттестацией ГИС или
оценкой соответствия ИСПДн. Здесь на него отвечает каждая находка:

```
[CRIT] C011 Незащищённый Docker Engine API опубликован наружу
       файл: docker-compose.yml:26
       мера: п. 63 з) — Защита программных интерфейсов взаимодействия приложений
       подмера: ЗПИ.2 — Управление доступом пользователей и приложений
       ПДн:  ЗСВ.1 — Идентификация и аутентификация в виртуальной инфраструктуре
       фикс: Не публикуйте сокет Docker наружу; используйте TLS (--tlsverify).
```

Привязка не декоративная: ссылки проверяются тестами против перечней,
выписанных из текстов документов. Правило не может сослаться на
несуществующий пункт или разойтись с его формулировкой.

> **Важно.** Инструмент не заменяет аттестацию и не является
> сертифицированным средством защиты информации. Соответствие находок
> пунктам приказов носит справочный характер: перед применением сверьте
> привязку с действующей редакцией документа, актом классификации и
> вашей моделью угроз.
...
Ссылки проверяются тестами: сослаться на несуществующую подмеру,
разойтись с формулировкой документа или указать неописанный пункт ГОСТа
правило не может. Полные
таблицы — в [`docs/measures-117.md`](docs/measures-117.md).

### Аутентификация: откуда берётся severity

**ГОСТ Р 58833-2020** «Защита информации. Идентификация и аутентификация.
Общие требования» (Росстандарт, 10.04.2020 N 159-ст) связывает вид
аутентификации с уровнем доверия к её результатам напрямую: простая
(однофакторная односторонняя) — низкий, усиленная — средний, строгая —
высокий (раздел 7.2). Пункт 6.9 прямо называет доступ по паролю
односторонней однофакторной аутентификацией.

Поэтому S002 (`PasswordAuthentication yes`) и P001 (`trust`/`md5`)
обоснованы не «здравым смыслом», а ссылкой: пароль → простая
аутентификация → низкий уровень доверия. Разбор — в
[`docs/authentication.md`](docs/authentication.md), включая
предупреждение о двух разных «уровнях доверия»: в этом ГОСТе и в
пункте 72 приказа N 117 это разные вещи.

### Персональные данные: приказ N 21

**Приказ ФСТЭК России от 18.02.2013 N 21** (в ред. приказов N 49 и N 68)
действует, и это второй нормативный контур проекта: для ГИС привязка
идёт по приказу N 117 и методичке, для ИСПДн — по приказу N 21. Каждое
правило несёт обе, в поле `pdn_measure` — код вида `ЗСВ.2`.

Приложение к приказу задаёт базовые наборы мер для четырёх уровней
защищённости, и флаг `--uz` сужает проверку до нужного:

```bash
fstec-lint . --uz 3          # базовый набор для УЗ3 — 30 правил из 40
fstec-lint --list-rules --uz 1
```

Мера, не отмеченная в базовом наборе, не является неприменимой: пункты
9 и 10 приказа предписывают адаптацию набора и компенсирующие меры.
Поэтому по умолчанию проверяются все правила.

Подробно, включая перечень исправленных привязок — в
[`docs/pdn.md`](docs/pdn.md).
...
Класс уязвимости, тип недостатка и место возникновения берутся из
**ГОСТ Р 56546** «Защита информации. Уязвимости информационных систем.
Классификация уязвимостей информационных систем» — стандарт 56545
требует этого в пунктах 5.2.6 и 5.2.10. Элементы, которые статический
анализ определить не может (версия ПО, аппаратная платформа, вектор
CVSS), помечаются отметкой «не определяется». Подробно — в
...
Приказ ФСТЭК N 117 делает разработку безопасного ПО отдельным
мероприятием (п. 34 м) и ссылается на **ГОСТ Р 56939-2024** «Защита
информации. Разработка безопасного программного обеспечения. Общие
требования». Стандарт описывает 25 процессов разработчика.

Самооценка проекта по этим процессам — в [`docs/rbpo.md`](docs/rbpo.md):
пять реализованы, десять частично, десять нет. Оценка сделана
разработчиком, внешний аудит не проводился. Крупнейшие пробелы названы
прямо: нет модели угроз самого инструмента (п. 5.7) и нет фаззинга
парсеров (п. 5.11) — при том, что пять парсеров разбирают недоверенный
ввод.
...
its **41 rules** to a specific clause of FSTEC Order No. 117 (state
