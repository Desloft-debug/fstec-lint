...
Находки — вход для расчёта показателя Кзи, но не сам показатель:
нормированные значения и методика берутся из методических документов
ФСТЭК, которых на дату в заголовке раздела нет.
...
меры до подмер. Правила дополнительно привязаны к его группам,
относящимся к конфигурации, архитектуре и развёртыванию:

| Группа | Наименование |
|---|---|
| `КК` | Контроль конфигураций информационных систем |
| `ПД` | Обеспечение защиты информации при предоставлении привилегированного доступа |
| `БР` | Обеспечение разработки безопасного программного обеспечения |
| `УД` | Обеспечение защиты информации при удалённом доступе |
| `ЗУ` | Обеспечение защиты информации при использовании конечных устройств |
| `НФ` | Обеспечение непрерывности функционирования |

Привязка на уровне группы, без номеров подмер: перечня подмер на руках
нет, а проставленный наугад номер хуже отсутствующего. Три правила
(P004, P005, P007 — регистрация событий и шифрование соединений) группы
не имеют сознательно: подходящих групп в имеющейся выписке нет. По
приказу N 117 они привязаны — п. 63 в) и п. 63 с).
...
- [ ] Детализация до подмер по методическим документам ФСТЭК (пункт 31
      Требований). Сейчас привязка на уровне пунктов приказа — это
      максимум, который проверяется по первоисточнику; методических
      документов с нормированными значениями Кзи и Пзи на 03.09.2026 нет
